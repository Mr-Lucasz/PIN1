...
exports.createBebida = async (nome_bebida, tipo_bebida, valor_bebida, imagem_bebida) => {
  const { data, error } = await supabase.from('Bebida').insert({ nome_bebida, tipo_bebida, valor_bebida, imagem_bebida }, { returning: 'minimal' }).select();
  if (error) {
...
exports.updateBebida = async (id_bebida, nome_bebida, tipo_bebida, valor_bebida, imagem_bebida) => {
  const { data, error } = await supabase.from('Bebida').update({ nome_bebida, tipo_bebida, valor_bebida, imagem_bebida }).eq('id_bebida', id_bebida).select();
  if (error) {
